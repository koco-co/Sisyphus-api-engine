...
class TestMockResponse:
    ...
    def test_to_dict(self):
        ...
        response = MockResponse(
            status_code=200, body={'key': 'value'}, headers={'X-Custom': 'test'}
        )
...
class TestRequestMatcher:
    ...
    def test_regex_path_match(self):
        ...
        matcher = RequestMatcher(
            method='GET', path=r'/api/users/\d+', match_type=MatchType.REGEX
        )
    ...
    def test_contains_path_match(self):
        ...
        matcher = RequestMatcher(
            method='GET', path='/users', match_type=MatchType.CONTAINS
        )
        assert matcher.matches('GET', '/api/users/123') is True
    ...
    def test_headers_match(self):
        ...
        matcher = RequestMatcher(
            method='GET', path='/api/users', headers={'Authorization': 'Bearer token'}
        )
        assert (
            matcher.matches(
        ...
        assert (
            matcher.matches(
                'GET', '/api/users', headers={'Authorization': 'Bearer other'}
            )
            is False
        )
        assert matcher.matches('GET', '/api/users') is False
    ...
    def test_body_exact_match(self):
        ...
        matcher = RequestMatcher(
            method='POST',
            path='/api/users',
            body_pattern='{"name":"John"}',
            body_match_type=MatchType.EXACT,
        )
...
class TestMockRule:
    ...
    def test_disabled_rule(self):
        ...
        response = MockResponse(status_code=200, body={})
        rule = MockRule(
            name='Get Users', matcher=matcher, response=response, enabled=False
        )
...
class TestMockServerConfig:
    ...
    def test_find_matching_rule(self):
        ...
        rule1 = MockRule(
            name='Get Users', matcher=matcher1, response=response1, priority=10
        )

        matcher2 = RequestMatcher(method='GET', path='/api/posts')
        response2 = MockResponse(status_code=200, body={'posts': []})
        rule2 = MockRule(
            name='Get Posts', matcher=matcher2, response=response2, priority=5
        )
    ...
    def test_priority_ordering(self):
        ...
        rule2 = MockRule(
            name='Rule 2', matcher=matcher, response=response2, priority=10
        )
...
class TestIntegration:
    ...
    def test_complete_flow(self):
        ...
        assert (
            config.find_matching_rule('GET', '/api/users/123').name == 'Get User by ID'
        )
        assert (
            config.find_matching_rule('POST', '/api/users', body='{"name":"John"}').name
            == 'Create User'
        )
        ...
        rule = MockRule(
            ...
            response=MockResponse(
                ...
                failure=FailureConfig(
                    failure_type=FailureType.HTTP_ERROR, probability=0.0
                ),
            ),
            priority=10,
            enabled=True,
