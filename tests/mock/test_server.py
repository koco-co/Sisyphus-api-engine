...
from apirun.mock.server import MockServer, create_regex_rule, create_simple_rule
    ...
    rules = [
        ...
        MockRule(
            ...
            matcher=RequestMatcher(
                method='GET', path='/api/search', query_params={'q': 'test'}
            ),
        ...
        MockRule(
            ...
            response=MockResponse(
                status_code=200, body={'authenticated': True, 'user': 'admin'}
            ),
        ),
        # Priority rules
        create_simple_rule(
        ...
        MockRule(
            ...
            matcher=RequestMatcher(
                ...
                path='/api/users',
                body_pattern='John',
                body_match_type=MatchType.CONTAINS,
            ),
            response=MockResponse(
                status_code=200, body={'created': True, 'matched': 'body contains John'}
            ),
            priority=3,
        ),
        # Complex match rule
        MockRule(
            ...
            response=MockResponse(
                status_code=201, body={'created': True, 'validated': True}
            ),
        ),
        # Temp rule for deletion test
        create_simple_rule(
            name='Temp Rule', method='GET', path='/api/temp', status_code=200
        ),
    ]
...
class TestMockServerBasic:
    ...
    def test_add_rule(self, mock_server_port):
        ...
        rule = create_simple_rule(
            name='New Rule', method='POST', path='/api/test', status_code=201
        )
    ...
    def test_remove_rule(self, mock_server_port):
        ...
        server = MockServer(config)
        rule = create_simple_rule(
            name='Test Rule', method='GET', path='/api/test', status_code=200
        )
        server.add_rule(rule)
        assert len(server.config.rules) == 1
    ...
    def test_list_rules(self, mock_server_port):
        ...
        rule1 = create_simple_rule(
            name='Rule 1', method='GET', path='/api/test1', status_code=200
        )
        rule2 = create_simple_rule(
            name='Rule 2', method='GET', path='/api/test2', status_code=200
        )
...
class TestMockServerIntegration:
    ...
    def test_post_request(self, running_server):
        ...
        response = requests.post(
            f'http://localhost:{port}/api/users', json={'name': 'Test'}
        )
    ...
    def test_header_matching(self, running_server):
        ...
        response = requests.get(
            f'http://localhost:{port}/api/protected',
            headers={'Authorization': 'Bearer token123'},
        )
        assert response.status_code == 200
...
class TestMockServerAdvanced:
    ...
    def setup_advanced_server(self, mock_server_port):
        ...
        rule3 = MockRule(
            ...
            response=MockResponse(
                status_code=200,
                body={'random': True},
                delay=DelayConfig(min_delay=0.05, max_delay=0.15),
            ),
        )
    ...
    def test_complex_matcher(self, setup_advanced_server):
        ...
        response = requests.post(
            f'http://localhost:{port}/api/complex',
            json={'test': 'data'},
            headers={'Content-Type': 'application/json'},
        )
        assert response.status_code == 404
    ...
    def test_body_matching(self, setup_advanced_server):
        ...
        # Should match
        response = requests.post(
            f'http://localhost:{port}/api/body', json={'name': 'John Doe'}
        )
        assert response.status_code == 200

        # Should not match
        response = requests.post(
            f'http://localhost:{port}/api/body', json={'name': 'Jane Doe'}
        )
        assert response.status_code == 404
