...
class TestJSONPathEnhancements:
    ...
    def test_filter_expression_not_equal(self):
        ...
        result = extract_value(
            "$.data.data[?(@.roleName != '项目所有者')].id", data, index=-1
        )
    ...
    def test_filter_expression_equals(self):
        ...
        # Extract admin names
        result = extract_value("$.users[?(@.role == 'admin')].name", data, index=-1)
        assert result == ['Alice', 'Charlie'], (
            f"Expected ['Alice', 'Charlie'], got {result}"
        )
    ...
    def test_combined_filter_and_projection(self):
        ...
        result = extract_value('$.items[?(@.price > 120)].id', data, index=-1)
        assert set(result) == {2, 3}, f'Expected {{2, 3}}, got {set(result)}'

        # Test active filter separately
        result = extract_value('$.items[?(@.active == true)].id', data, index=-1)
    ...
    def test_filter_with_string_comparison(self):
        ...
        # Extract admin IDs
        result = extract_value("$.users[?(@.role == 'admin')].id", data, index=-1)
        assert result == [1, 3], f'Expected [1, 3], got {result}'
...
class TestVariableNestedReference:
    ...
    def test_nested_variable_reference(self):
        ...
        assert result == 'test_param_0129', (
            f"Expected 'test_param_0129', got '{result}'"
        )
...
class TestMicrosecondTimestamp:
    ...
    def test_now_with_microseconds(self):
        ...
        # Should be 6 digits
        assert len(microsecond_str) == 6, (
            f"Expected 6 digits, got {len(microsecond_str)}: '{microsecond_str}'"
        )
        assert microsecond_str.isdigit(), (
            f"Expected all digits, got: '{microsecond_str}'"
        )
