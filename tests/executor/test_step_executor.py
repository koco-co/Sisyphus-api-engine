...
from apirun.core.models import ErrorCategory, Extractor, StepResult, TestStep
...
class TestStepExecutorExtraction:
    ...
    def test_extract_variables_from_response_body_new_style(self):
        ...
        step = TestStep(
            ...
            extractors=[
                ...
                Extractor(
                    name='first_id', type='jsonpath', path='$.json.data.data[0].id'
                ),
            ],
        )
    ...
    def test_extract_variables_backward_compatible_old_style(self):
        ...
        mock_response = {
            'status_code': 200,
            'body': {'json': {'code': 'SUCCESS', 'message': 'OK'}},
        }
    ...
    def test_extract_variables_nested_array_index(self):
        ...
        step = TestStep(
            ...
            extractors=[
                ...
                Extractor(
                    name='second_id', type='jsonpath', path='$.json.data.data[1].id'
                ),
                Extractor(
                    name='third_name', type='jsonpath', path='$.json.data.data[2].name'
                ),
            ],
        )
...
class TestStepExecutorConditionEvaluation:
    ...
    def test_evaluate_simple_condition_contains(self):
        ...
        result = StepExecutor._evaluate_simple_condition(
            "$.tags contains 'python'", data_source
        )
        assert result is True

        result = StepExecutor._evaluate_simple_condition(
            "$.tags contains 'java'", data_source
        )
        assert result is False
    ...
    def test_evaluate_simple_condition_in(self):
        ...
        result = StepExecutor._evaluate_simple_condition(
            "$.status in 'active status'", data_source
        )
    ...
    def test_evaluate_simple_condition_not_in(self):
        ...
        result = StepExecutor._evaluate_simple_condition(
            "$.status not in ['inactive', 'deleted']", data_source
        )
        assert result is True
    ...
    def test_evaluate_condition_with_and(self):
        ...
        result = StepExecutor._evaluate_condition(
            "$.code == 200 and $.status == 'ok'", data_source
        )
        assert result is True

        result = StepExecutor._evaluate_condition(
            "$.code == 200 and $.status == 'error'", data_source
        )
        assert result is False
    ...
    def test_evaluate_condition_with_or(self):
        ...
        result = StepExecutor._evaluate_condition(
            "$.code == 404 or $.status == 'ok'", data_source
        )
        assert result is True

        result = StepExecutor._evaluate_condition(
            "$.code == 404 or $.status == 'error'", data_source
        )
        assert result is False
    ...
    def test_evaluate_condition_with_parentheses(self):
        ...
        result = StepExecutor._evaluate_condition(
            '$.a == 1 and ($.b == 2 or $.c == 5)', data_source
        )
        assert result is True
