...
class TestProfileConfig:
    ...
    def test_profile_config_creation(self):
        ...
        config = ProfileConfig(
            base_url='https://api.example.com', timeout=60, verify_ssl=False
        )
...
class TestValidationRule:
    ...
    def test_validation_rule_creation(self):
        """测试验证规则创建。"""
        rule = ValidationRule(
            type='eq', path='$.status', expect=200, description='验证状态码为200'
        )
        assert rule.type == 'eq'
...
class TestExtractor:
    ...
    def test_jsonpath_extractor_creation(self):
        """测试 JSONPath 提取器创建。"""
        extractor = Extractor(name='user_id', type='jsonpath', path='$.data.user.id')
    ...
    def test_regex_extractor(self):
        """测试正则表达式提取器。"""
        extractor = Extractor(
            name='order_id', type='regex', path='Order ID: (\\d+)', index=1
        )
...
class TestModel:
    ...
    def test_test_case_with_steps(self):
        ...
        case = TestCase(name='多步骤测试', steps=steps)
    ...
    def test_test_case_with_config(self):
        """测试带配置的测试用例。"""
        config = GlobalConfig(
            name='测试配置', variables={'base_url': 'https://api.test.com'}
        )
        case = TestCase(name='配置测试', config=config)
        assert case.config is not None
        assert case.config.variables['base_url'] == 'https://api.test.com'

    def test_test_case_with_tags(self):
        """测试带标签的测试用例。"""
        case = TestCase(name='标签测试', tags=['smoke', 'critical', 'auth'])
...
class TestErrorInfo:
    ...
    def test_error_info_creation(self):
        """测试错误信息创建。"""
        error = ErrorInfo(
            type='ValueError', category=ErrorCategory.BUSINESS, message='无效的参数值'
        )
...
class TestPerformanceMetrics:
    ...
    def test_performance_metrics_creation(self):
        ...
        metrics = PerformanceMetrics(
            total_time=1500.0, dns_time=50.0, tcp_time=100.0, tls_time=200.0
        )
...
class TestStepResult:
    ...
    def test_step_result_creation(self):
        """测试步骤结果创建。"""
        result = StepResult(name='测试步骤', status='success')
    ...
    def test_step_result_with_performance(self):
        ...
        result = StepResult(name='性能测试', status='success', performance=performance)
    ...
    def test_step_result_with_error(self):
        """测试带错误信息的步骤结果。"""
        error = ErrorInfo(
            type='TimeoutError', category=ErrorCategory.TIMEOUT, message='请求超时'
        )
        result = StepResult(name='超时步骤', status='error', error_info=error)
