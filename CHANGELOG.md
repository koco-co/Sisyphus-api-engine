...
- **安全防护引擎 (`apirun/security`)**：引入 SQL 注入防护验证、正则验证机制 (防止 ReDoS)、响应 Size Limiter 及敏感日志自动脱敏。
- **稳定性增强 (`apirun/utils`)**：增加 `retry.py` 和 `timeout.py` ，提供底层的重试与全局超时容错机制。
- **并发与性能测试 (`tests/performance`)**：测试套件新增针对多线程的高并发发包以及处理效能进行性能评估的基准测试；依赖添加了 `pytest-benchmark` 和 `pytest-xdist`。

### 变更

- HTTP 请求执行器整合重试机制，应对网络波动等可能导致连接失败的场景。
- 升级引擎日志系统以集成脱敏模块，保护运行期间的数据隐私。
