"""HTML Report Exporter for Sisyphus API Engine.

This module implements HTML report generation with interactive features.
Following Google Python Style Guide.
"""

import json
from typing import Any, Dict, List
from datetime import datetime
from pathlib import Path

from apirun.core.models import TestCase, TestCaseResult, StepResult


class HTMLExporter:
    """Export test results to HTML format.

    Features:
    - Responsive design
    - Interactive expand/collapse sections
    - Color-coded status indicators
    - Detailed step information
    - Performance metrics visualization
    - Error details and stack traces
    - Variable state tracking
    - Multi-language support (Chinese/English)
    - Execution timeline visualization ⭐
    - Variable flow diagram ⭐
    - Dependency relationship graph ⭐
    - Performance analysis charts ⭐
    """

    # 国际化文本字典
    I18N_TEXTS = {
        "en": {
            "title": "Sisyphus API Test Report",
            "test_case": "Test Case",
            "generated": "Generated",
            "status": "Status",
            "test_summary": "Test Summary",
            "duration": "Duration",
            "started": "Started",
            "completed": "Completed",
            "seconds": "seconds",
            "statistics": "Statistics",
            "total_steps": "Total Steps",
            "passed": "Passed ✓",
            "failed": "Failed ✗",
            "skipped": "Skipped ⊘",
            "test_steps": "Test Steps",
            "expand_all": "Expand All",
            "collapse_all": "Collapse All",
            "performance_metrics": "Performance Metrics",
            "response": "Response",
            "validations": "Validations",
            "extracted_variables": "Extracted Variables",
            "variables_snapshot": "Variables Snapshot",
            "final_variables": "Final Variables",
            "total_time": "Total Time",
            "dns_time": "DNS Time",
            "tcp_time": "TCP Time",
            "tls_time": "TLS Time",
            "server_time": "Server Time",
            "download_time": "Download Time",
            "size": "Size",
            "bytes": "bytes",
            "suggestion": "Suggestion",
            "generated_by": "Generated by",
            "footer_text": "Enterprise-grade API Testing",
            "retries": "Retries",
            "execution_timeline": "Execution Timeline",
            "variable_flow": "Variable Flow",
            "dependency_graph": "Dependency Graph",
            "performance_analysis": "Performance Analysis",
            "timeline_description": "Visualize step execution timeline",
            "variable_flow_description": "Track variable creation and modifications",
            "dependency_description": "Show step dependencies based on variable usage",
            "performance_description": "Analyze performance metrics and identify bottlenecks",
            "response_time_distribution": "Response Time Distribution",
            "bottleneck_analysis": "Bottleneck Analysis",
            "slowest_steps": "Slowest Steps",
            "fastest_steps": "Fastest Steps",
            "variable_created": "Created",
            "variable_modified": "Modified",
            "variable_used": "Used",
            "step_depends_on": "depends on",
            "no_dependencies": "No dependencies detected",
            "timeline_seconds": "s",
            "average_response_time": "Average Response Time",
            "total_execution_time": "Total Execution Time",
        },
        "zh": {
            "title": "Sisyphus API 测试报告",
            "test_case": "测试用例",
            "generated": "生成时间",
            "status": "状态",
            "test_summary": "测试摘要",
            "duration": "持续时间",
            "started": "开始时间",
            "completed": "完成时间",
            "seconds": "秒",
            "statistics": "统计数据",
            "total_steps": "总步骤数",
            "passed": "通过 ✓",
            "failed": "失败 ✗",
            "skipped": "跳过 ⊘",
            "test_steps": "测试步骤",
            "expand_all": "全部展开",
            "collapse_all": "全部折叠",
            "performance_metrics": "性能指标",
            "response": "响应",
            "validations": "验证结果",
            "extracted_variables": "提取的变量",
            "variables_snapshot": "变量快照",
            "final_variables": "最终变量",
            "total_time": "总耗时",
            "dns_time": "DNS 解析",
            "tcp_time": "TCP 连接",
            "tls_time": "TLS 握手",
            "server_time": "服务器处理",
            "download_time": "下载时间",
            "size": "大小",
            "bytes": "字节",
            "suggestion": "建议",
            "generated_by": "生成工具",
            "footer_text": "企业级 API 测试平台",
            "retries": "重试次数",
            "execution_timeline": "执行时序图",
            "variable_flow": "变量流转图",
            "dependency_graph": "依赖关系图",
            "performance_analysis": "性能分析",
            "timeline_description": "可视化步骤执行时间线",
            "variable_flow_description": "追踪变量创建和修改过程",
            "dependency_description": "基于变量使用显示步骤依赖关系",
            "performance_description": "分析性能指标并识别瓶颈",
            "response_time_distribution": "响应时间分布",
            "bottleneck_analysis": "瓶颈分析",
            "slowest_steps": "最慢步骤",
            "fastest_steps": "最快步骤",
            "variable_created": "创建",
            "variable_modified": "修改",
            "variable_used": "使用",
            "step_depends_on": "依赖于",
            "no_dependencies": "未检测到依赖关系",
            "timeline_seconds": "秒",
            "average_response_time": "平均响应时间",
            "total_execution_time": "总执行时间",
        }
    }

    def __init__(self, theme: str = "light", language: str = "en"):
        """Initialize HTMLExporter.

        Args:
            theme: Theme for the report (light/dark)
            language: Language for the report (en/zh)
        """
        self.theme = theme
        self.language = language if language in ["en", "zh"] else "en"
        self.colors = self._get_theme_colors(theme)
        self.texts = self.I18N_TEXTS[self.language]

    def _get_theme_colors(self, theme: str) -> Dict[str, str]:
        """Get color scheme for the theme.

        Args:
            theme: Theme name

        Returns:
            Color scheme dictionary
        """
        if theme == "dark":
            return {
                "bg_primary": "#1e1e1e",
                "bg_secondary": "#2d2d2d",
                "bg_tertiary": "#3d3d3d",
                "text_primary": "#e0e0e0",
                "text_secondary": "#a0a0a0",
                "success": "#4caf50",
                "failure": "#f44336",
                "skipped": "#ff9800",
                "border": "#404040",
                "accent": "#2196f3",
            }
        else:
            return {
                "bg_primary": "#ffffff",
                "bg_secondary": "#f5f5f5",
                "bg_tertiary": "#e0e0e0",
                "text_primary": "#333333",
                "text_secondary": "#666666",
                "success": "#4caf50",
                "failure": "#f44336",
                "skipped": "#ff9800",
                "border": "#dddddd",
                "accent": "#2196f3",
            }

    def to_html(self, result: TestCaseResult) -> str:
        """Convert test result to HTML format.

        Args:
            result: Test case result

        Returns:
            HTML string
        """
        # Build HTML components
        html_parts = []

        # HTML header
        html_parts.append(self._build_html_header())

        # CSS styles
        html_parts.append(self._build_css_styles())

        # Body start
        html_parts.append(self._build_body_start())

        # Header section
        html_parts.append(self._build_header_section(result))

        # Summary section
        html_parts.append(self._build_summary_section(result))

        # Statistics section
        html_parts.append(self._build_statistics_section(result))

        # Execution Timeline section
        html_parts.append(self._build_timeline_section(result))

        # Variable Flow section
        html_parts.append(self._build_variable_flow_section(result))

        # Dependency Graph section
        html_parts.append(self._build_dependency_section(result))

        # Performance Analysis section
        html_parts.append(self._build_performance_analysis_section(result))

        # Steps section
        html_parts.append(self._build_steps_section(result))

        # Variables section
        if result.final_variables:
            html_parts.append(self._build_variables_section(result))

        # Footer
        html_parts.append(self._build_footer())

        # JavaScript
        html_parts.append(self._build_javascript())

        # Body end
        html_parts.append("</body></html>")

        return "\n".join(html_parts)

    def _build_html_header(self) -> str:
        """Build HTML document header.

        Returns:
            HTML header string
        """
        lang_attr = "zh" if self.language == "zh" else "en"
        return f"""<!DOCTYPE html>
<html lang="{lang_attr}">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{self.texts['title']}</title>
    <style>"""

    def _build_css_styles(self) -> str:
        """Build CSS styles for the report.

        Returns:
            CSS styles string
        """
        c = self.colors
        return f"""
        * {{
            margin: 0;
            padding: 0;
            box-sizing: border-box;
        }}

        body {{
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif;
            background-color: {c['bg_primary']};
            color: {c['text_primary']};
            line-height: 1.6;
            padding: 20px;
        }}

        .container {{
            max-width: 1200px;
            margin: 0 auto;
        }}

        .header {{
            background: linear-gradient(135deg, {c['accent']} 0%, #1976d2 100%);
            color: white;
            padding: 30px;
            border-radius: 8px;
            margin-bottom: 30px;
            box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
        }}

        .header h1 {{
            font-size: 28px;
            margin-bottom: 10px;
        }}

        .header .meta {{
            font-size: 14px;
            opacity: 0.9;
        }}

        .section {{
            background-color: {c['bg_secondary']};
            border-radius: 8px;
            padding: 25px;
            margin-bottom: 20px;
            border: 1px solid {c['border']};
        }}

        .section h2 {{
            font-size: 20px;
            margin-bottom: 20px;
            color: {c['text_primary']};
            border-bottom: 2px solid {c['border']};
            padding-bottom: 10px;
        }}

        .status-badge {{
            display: inline-block;
            padding: 6px 12px;
            border-radius: 4px;
            font-weight: 600;
            font-size: 14px;
            text-transform: uppercase;
        }}

        .status-passed {{
            background-color: {c['success']};
            color: white;
        }}

        .status-failed {{
            background-color: {c['failure']};
            color: white;
        }}

        .status-skipped {{
            background-color: {c['skipped']};
            color: white;
        }}

        .stats-grid {{
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));
            gap: 20px;
            margin-top: 20px;
        }}

        .stat-card {{
            background-color: {c['bg_primary']};
            padding: 20px;
            border-radius: 6px;
            text-align: center;
            border: 1px solid {c['border']};
        }}

        .stat-card .value {{
            font-size: 36px;
            font-weight: bold;
            margin-bottom: 5px;
        }}

        .stat-card .label {{
            font-size: 14px;
            color: {c['text_secondary']};
            text-transform: uppercase;
        }}

        .stat-card.passed .value {{
            color: {c['success']};
        }}

        .stat-card.failed .value {{
            color: {c['failure']};
        }}

        .stat-card.skipped .value {{
            color: {c['skipped']};
        }}

        .step-item {{
            background-color: {c['bg_primary']};
            border-radius: 6px;
            margin-bottom: 15px;
            border: 1px solid {c['border']};
            overflow: hidden;
        }}

        .step-header {{
            padding: 15px 20px;
            display: flex;
            align-items: center;
            justify-content: space-between;
            cursor: pointer;
            transition: background-color 0.2s;
        }}

        .step-header:hover {{
            background-color: {c['bg_tertiary']};
        }}

        .step-info {{
            display: flex;
            align-items: center;
            gap: 15px;
        }}

        .step-icon {{
            width: 24px;
            height: 24px;
            border-radius: 50%;
            display: flex;
            align-items: center;
            justify-content: center;
            font-size: 12px;
            font-weight: bold;
            color: white;
        }}

        .step-name {{
            font-weight: 600;
            font-size: 16px;
        }}

        .step-meta {{
            display: flex;
            align-items: center;
            gap: 20px;
            color: {c['text_secondary']};
            font-size: 14px;
        }}

        .step-details {{
            display: none;
            padding: 20px;
            border-top: 1px solid {c['border']};
            background-color: {c['bg_secondary']};
        }}

        .step-details.active {{
            display: block;
        }}

        .detail-section {{
            margin-bottom: 20px;
        }}

        .detail-section h4 {{
            font-size: 14px;
            margin-bottom: 10px;
            color: {c['text_secondary']};
            text-transform: uppercase;
        }}

        .detail-content {{
            background-color: {c['bg_primary']};
            padding: 15px;
            border-radius: 4px;
            border: 1px solid {c['border']};
            font-family: 'Courier New', monospace;
            font-size: 13px;
            overflow-x: auto;
            white-space: pre-wrap;
            word-wrap: break-word;
        }}

        .error-box {{
            background-color: #ffebee;
            border-left: 4px solid {c['failure']};
            padding: 15px;
            border-radius: 4px;
            margin-bottom: 15px;
        }}

        .error-box.dark {{
            background-color: #3d1e1e;
        }}

        .error-title {{
            font-weight: 600;
            color: {c['failure']};
            margin-bottom: 10px;
        }}

        .performance-metrics {{
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(150px, 1fr));
            gap: 10px;
        }}

        .metric-item {{
            display: flex;
            justify-content: space-between;
            padding: 8px;
            background-color: {c['bg_primary']};
            border-radius: 4px;
            font-size: 13px;
        }}

        .metric-label {{
            color: {c['text_secondary']};
        }}

        .metric-value {{
            font-weight: 600;
        }}

        .variables-grid {{
            display: grid;
            grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));
            gap: 10px;
        }}

        .variable-item {{
            background-color: {c['bg_primary']};
            padding: 12px;
            border-radius: 4px;
            border: 1px solid {c['border']};
        }}

        .variable-name {{
            font-weight: 600;
            color: {c['accent']};
            margin-bottom: 5px;
        }}

        .variable-value {{
            font-family: 'Courier New', monospace;
            font-size: 13px;
            color: {c['text_secondary']};
            word-wrap: break-word;
        }}

        .progress-bar {{
            width: 100%;
            height: 30px;
            background-color: {c['bg_tertiary']};
            border-radius: 15px;
            overflow: hidden;
            display: flex;
            margin-top: 20px;
        }}

        .progress-segment {{
            height: 100%;
            display: flex;
            align-items: center;
            justify-content: center;
            color: white;
            font-weight: 600;
            font-size: 14px;
            transition: width 0.3s;
        }}

        .progress-passed {{
            background-color: {c['success']};
        }}

        .progress-failed {{
            background-color: {c['failure']};
        }}

        .progress-skipped {{
            background-color: {c['skipped']};
        }}

        .footer {{
            text-align: center;
            padding: 20px;
            color: {c['text_secondary']};
            font-size: 14px;
            margin-top: 40px;
        }}

        .toggle-all {{
            margin-bottom: 15px;
        }}

        .toggle-all button {{
            padding: 8px 16px;
            margin-right: 10px;
            border: 1px solid {c['border']};
            background-color: {c['bg_primary']};
            color: {c['text_primary']};
            border-radius: 4px;
            cursor: pointer;
            font-size: 14px;
            transition: all 0.2s;
        }}

        .toggle-all button:hover {{
            background-color: {c['bg_tertiary']};
        }}

        /* Section description */
        .section-description {{
            color: {c['text_secondary']};
            font-size: 14px;
            margin-bottom: 20px;
        }}

        .no-data {{
            color: {c['text_secondary']};
            font-style: italic;
            text-align: center;
            padding: 20px;
        }}

        /* Timeline styles */
        .timeline-container {{
            margin-top: 20px;
        }}

        .timeline-chart {{
            margin-bottom: 15px;
        }}

        .timeline-bar-wrapper {{
            display: grid;
            grid-template-columns: 50px 1fr 80px;
            gap: 10px;
            align-items: center;
            margin-bottom: 12px;
        }}

        .timeline-bar-label {{
            font-size: 12px;
            color: {c['text_secondary']};
            overflow: hidden;
            text-overflow: ellipsis;
            white-space: nowrap;
        }}

        .timeline-bar {{
            position: relative;
            height: 30px;
            background-color: {c['bg_tertiary']};
            border-radius: 4px;
            overflow: hidden;
        }}

        .timeline-bar-fill {{
            position: absolute;
            height: 100%;
            border-radius: 4px;
            transition: width 0.3s;
            min-width: 2px;
        }}

        .timeline-bar-time {{
            font-size: 12px;
            color: {c['text_secondary']};
            text-align: right;
        }}

        .timeline-legend {{
            display: flex;
            gap: 20px;
            justify-content: center;
            margin-top: 15px;
        }}

        .legend-item {{
            display: flex;
            align-items: center;
            gap: 8px;
            font-size: 13px;
        }}

        .legend-color {{
            width: 16px;
            height: 16px;
            border-radius: 3px;
        }}

        /* Variable flow styles */
        .variable-flow-container {{
            margin-top: 20px;
            max-height: 400px;
            overflow-y: auto;
        }}

        .variable-flow-event {{
            display: grid;
            grid-template-columns: 60px 80px 150px 1fr;
            gap: 10px;
            padding: 10px;
            background-color: {c['bg_primary']};
            border-radius: 4px;
            margin-bottom: 8px;
            font-size: 13px;
            align-items: center;
        }}

        .variable-flow-step {{
            font-weight: 600;
            color: {c['accent']};
        }}

        .variable-flow-action {{
            font-weight: 600;
        }}

        .variable-flow-name {{
            font-family: 'Courier New', monospace;
            color: {c['text_primary']};
        }}

        .variable-flow-value {{
            color: {c['text_secondary']};
            overflow: hidden;
            text-overflow: ellipsis;
            white-space: nowrap;
        }}

        .variable-flow-more {{
            text-align: center;
            color: {c['text_secondary']};
            font-style: italic;
            padding: 10px;
        }}

        /* Dependency graph styles */
        .dependency-container {{
            margin-top: 20px;
        }}

        .dependency-item {{
            background-color: {c['bg_primary']};
            padding: 12px;
            border-radius: 4px;
            margin-bottom: 10px;
            border-left: 3px solid {c['accent']};
        }}

        .dependency-step {{
            font-size: 14px;
            margin-bottom: 5px;
        }}

        .dependency-deps {{
            font-size: 13px;
            color: {c['text_secondary']};
        }}

        /* Performance analysis styles */
        .performance-overview {{
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(150px, 1fr));
            gap: 15px;
            margin-bottom: 25px;
        }}

        .perf-stat-card {{
            background-color: {c['bg_primary']};
            padding: 15px;
            border-radius: 6px;
            text-align: center;
            border: 1px solid {c['border']};
        }}

        .perf-stat-value {{
            font-size: 28px;
            font-weight: bold;
            color: {c['accent']};
            margin-bottom: 5px;
        }}

        .perf-stat-label {{
            font-size: 12px;
            color: {c['text_secondary']};
            text-transform: uppercase;
        }}

        .subsection-title {{
            font-size: 16px;
            margin-top: 20px;
            margin-bottom: 12px;
            color: {c['text_primary']};
        }}

        .performance-list {{
            background-color: {c['bg_primary']};
            border-radius: 4px;
            padding: 12px;
            margin-bottom: 15px;
        }}

        .perf-list-item {{
            padding: 8px 0;
            border-bottom: 1px solid {c['border']};
            font-size: 13px;
        }}

        .perf-list-item:last-child {{
            border-bottom: none;
        }}

        .response-time-chart {{
            margin-top: 15px;
        }}

        .perf-bar-item {{
            display: grid;
            grid-template-columns: 1fr 200px 80px;
            gap: 10px;
            align-items: center;
            margin-bottom: 10px;
        }}

        .perf-bar-label {{
            font-size: 12px;
            color: {c['text_secondary']};
            overflow: hidden;
            text-overflow: ellipsis;
            white-space: nowrap;
        }}

        .perf-bar-track {{
            height: 24px;
            background-color: {c['bg_tertiary']};
            border-radius: 4px;
            overflow: hidden;
        }}

        .perf-bar-fill {{
            height: 100%;
            background: linear-gradient(90deg, {c['accent']} 0%, #1976d2 100%);
            border-radius: 4px;
            transition: width 0.3s;
        }}

        .perf-bar-value {{
            font-size: 12px;
            color: {c['text_secondary']};
            text-align: right;
        }}

        @media (max-width: 768px) {{
            .stats-grid {{
                grid-template-columns: 1fr;
            }}

            .step-meta {{
                flex-direction: column;
                align-items: flex-start;
                gap: 5px;
            }}

            .timeline-bar-wrapper {{
                grid-template-columns: 1fr;
                gap: 5px;
            }}

            .variable-flow-event {{
                grid-template-columns: 1fr;
                gap: 5px;
            }}

            .perf-bar-item {{
                grid-template-columns: 1fr;
                gap: 5px;
            }}
        }}
    </style>"""

    def _build_body_start(self) -> str:
        """Build body start tag.

        Returns:
            Body start string
        """
        return "</head><body><div class='container'>"

    def _build_header_section(self, result: TestCaseResult) -> str:
        """Build header section.

        Args:
            result: Test case result

        Returns:
            Header HTML string
        """
        status_class = f"status-{result.status}"

        return f"""
        <div class="header">
            <h1>🧪 {self.texts['title']}</h1>
            <div class="meta">
                <strong>{self.texts['test_case']}:</strong> {self._escape_html(result.name)}<br>
                <strong>{self.texts['generated']}:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}<br>
                <strong>{self.texts['status']}:</strong> <span class="status-badge {status_class}">{result.status.upper()}</span>
            </div>
        </div>"""

    def _build_summary_section(self, result: TestCaseResult) -> str:
        """Build summary section.

        Args:
            result: Test case result

        Returns:
            Summary HTML string
        """
        pass_rate = (result.passed_steps / result.total_steps * 100) if result.total_steps > 0 else 0

        return f"""
        <div class="section">
            <h2>📊 {self.texts['test_summary']}</h2>
            <p><strong>{self.texts['duration']}:</strong> {result.duration:.2f} {self.texts['seconds']}</p>
            <p><strong>{self.texts['started']}:</strong> {result.start_time.strftime('%Y-%m-%d %H:%M:%S')}</p>
            <p><strong>{self.texts['completed']}:</strong> {result.end_time.strftime('%Y-%m-%d %H:%M:%S')}</p>

            <div class="progress-bar">
                <div class="progress-segment progress-passed" style="width: {pass_rate}%">
                    {pass_rate:.1f}% {self.texts['passed'].split(' ')[0]}
                </div>
                <div class="progress-segment progress-failed" style="width: {(result.failed_steps / result.total_steps * 100) if result.total_steps > 0 else 0}%">
                    {(result.failed_steps / result.total_steps * 100) if result.total_steps > 0 else 0:.1f}% {self.texts['failed'].split(' ')[0]}
                </div>
                <div class="progress-segment progress-skipped" style="width: {(result.skipped_steps / result.total_steps * 100) if result.total_steps > 0 else 0}%">
                    {(result.skipped_steps / result.total_steps * 100) if result.total_steps > 0 else 0:.1f}% {self.texts['skipped'].split(' ')[0]}
                </div>
            </div>
        </div>"""

    def _build_statistics_section(self, result: TestCaseResult) -> str:
        """Build statistics section.

        Args:
            result: Test case result

        Returns:
            Statistics HTML string
        """
        return f"""
        <div class="section">
            <h2>📈 {self.texts['statistics']}</h2>
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="value">{result.total_steps}</div>
                    <div class="label">{self.texts['total_steps']}</div>
                </div>
                <div class="stat-card passed">
                    <div class="value">{result.passed_steps}</div>
                    <div class="label">{self.texts['passed']}</div>
                </div>
                <div class="stat-card failed">
                    <div class="value">{result.failed_steps}</div>
                    <div class="label">{self.texts['failed']}</div>
                </div>
                <div class="stat-card skipped">
                    <div class="value">{result.skipped_steps}</div>
                    <div class="label">{self.texts['skipped']}</div>
                </div>
            </div>
        </div>"""

    def _build_steps_section(self, result: TestCaseResult) -> str:
        """Build steps section.

        Args:
            result: Test case result

        Returns:
            Steps HTML string
        """
        html = f"""
        <div class="section">
            <h2>📋 {self.texts['test_steps']}</h2>
            <div class="toggle-all">
                <button onclick="expandAll()">{self.texts['expand_all']}</button>
                <button onclick="collapseAll()">{self.texts['collapse_all']}</button>
            </div>
        """

        for idx, step in enumerate(result.step_results, 1):
            html += self._build_step_item(step, idx)

        html += "</div>"
        return html

    def _build_step_item(self, step: StepResult, index: int) -> str:
        """Build a single step item.

        Args:
            step: Step result
            index: Step index

        Returns:
            Step HTML string
        """
        status_icon = {
            "success": "✓",
            "failure": "✗",
            "skipped": "⊘",
            "error": "⚠",
        }.get(step.status, "?")

        status_color = {
            "success": self.colors["success"],
            "failure": self.colors["failure"],
            "skipped": self.colors["skipped"],
            "error": self.colors["failure"],
        }.get(step.status, self.colors["text_secondary"])

        # Calculate duration
        duration = 0.0
        if step.start_time and step.end_time:
            duration = (step.end_time - step.start_time).total_seconds()

        return f"""
        <div class="step-item">
            <div class="step-header" onclick="toggleStep(this)">
                <div class="step-info">
                    <div class="step-icon" style="background-color: {status_color}">
                        {status_icon}
                    </div>
                    <div class="step-name">{self._escape_html(step.name)}</div>
                </div>
                <div class="step-meta">
                    <div><strong>{self.texts['status']}:</strong> {step.status.upper()}</div>
                    <div><strong>{self.texts['duration']}:</strong> {duration:.3f}s</div>
                    {f'<div><strong>{self.texts["retries"]}:</strong> {step.retry_count}</div>' if step.retry_count > 0 else ''}
                </div>
            </div>
            <div class="step-details">
                {self._build_step_details(step)}
            </div>
        </div>"""

    def _build_step_details(self, step: StepResult) -> str:
        """Build step details section.

        Args:
            step: Step result

        Returns:
            Step details HTML string
        """
        details = []

        # Error information
        if step.error_info:
            details.append(f"""
            <div class="error-box{' dark' if self.theme == 'dark' else ''}">
                <div class="error-title">❌ {self._escape_html(step.error_info.type)}</div>
                <div>{self._escape_html(step.error_info.message)}</div>
                {f'<div style="margin-top: 10px;"><strong>{self.texts["suggestion"]}:</strong> {self._escape_html(step.error_info.suggestion)}</div>' if step.error_info.suggestion else ''}
            </div>
            """)

        # Performance metrics
        if step.performance:
            perf = step.performance
            details.append(f"""
            <div class="detail-section">
                <h4>⚡ {self.texts['performance_metrics']}</h4>
                <div class="performance-metrics">
                    <div class="metric-item">
                        <span class="metric-label">{self.texts['total_time']}</span>
                        <span class="metric-value">{perf.total_time:.2f}ms</span>
                    </div>
                    {f'<div class="metric-item"><span class="metric-label">{self.texts["dns_time"]}</span><span class="metric-value">{perf.dns_time:.2f}ms</span></div>' if perf.dns_time > 0 else ''}
                    {f'<div class="metric-item"><span class="metric-label">{self.texts["tcp_time"]}</span><span class="metric-value">{perf.tcp_time:.2f}ms</span></div>' if perf.tcp_time > 0 else ''}
                    {f'<div class="metric-item"><span class="metric-label">{self.texts["tls_time"]}</span><span class="metric-value">{perf.tls_time:.2f}ms</span></div>' if perf.tls_time > 0 else ''}
                    {f'<div class="metric-item"><span class="metric-label">{self.texts["server_time"]}</span><span class="metric-value">{perf.server_time:.2f}ms</span></div>' if perf.server_time > 0 else ''}
                    {f'<div class="metric-item"><span class="metric-label">{self.texts["download_time"]}</span><span class="metric-value">{perf.download_time:.2f}ms</span></div>' if perf.download_time > 0 else ''}
                    {f'<div class="metric-item"><span class="metric-label">{self.texts["size"]}</span><span class="metric-value">{perf.size} {self.texts["bytes"]}</span></div>' if perf.size > 0 else ''}
                </div>
            </div>
            """)

        # Response data
        if step.response:
            details.append(f"""
            <div class="detail-section">
                <h4>📡 {self.texts['response']}</h4>
                <div class="detail-content">{self._escape_html(json.dumps(step.response, indent=2, ensure_ascii=False))}</div>
            </div>
            """)

        # Validation results
        if step.validation_results:
            validations_html = f"<div class='detail-section'><h4>✓ {self.texts['validations']}</h4><ul>"
            for v in step.validation_results:
                icon = "✓" if v.get("passed", True) else "✗"
                color = self.colors["success"] if v.get("passed", True) else self.colors["failure"]
                validations_html += f"<li style='color: {color};'>{icon} {self._escape_html(v.get('description', 'N/A'))}</li>"
            validations_html += "</ul></div>"
            details.append(validations_html)

        # Extracted variables
        if step.extracted_vars:
            details.append(f"""
            <div class="detail-section">
                <h4>🔍 {self.texts['extracted_variables']}</h4>
                <div class="variables-grid">
                    {"".join(f"<div class='variable-item'><div class='variable-name'>{self._escape_html(k)}</div><div class='variable-value'>{self._escape_html(str(v))}</div></div>" for k, v in step.extracted_vars.items())}
                </div>
            </div>
            """)

        # Variables snapshot
        if step.variables_snapshot:
            details.append(f"""
            <div class="detail-section">
                <h4>📸 {self.texts['variables_snapshot']}</h4>
                <div class="detail-content">{self._escape_html(json.dumps(step.variables_snapshot, indent=2, ensure_ascii=False))}</div>
            </div>
            """)

        return "".join(details)

    def _build_variables_section(self, result: TestCaseResult) -> str:
        """Build variables section.

        Args:
            result: Test case result

        Returns:
            Variables HTML string
        """
        return f"""
        <div class="section">
            <h2>🔧 {self.texts['final_variables']}</h2>
            <div class="variables-grid">
                {"".join(f"<div class='variable-item'><div class='variable-name'>{self._escape_html(k)}</div><div class='variable-value'>{self._escape_html(str(v))}</div></div>" for k, v in result.final_variables.items())}
            </div>
        </div>
        """

    def _build_timeline_section(self, result: TestCaseResult) -> str:
        """Build execution timeline section.

        Args:
            result: Test case result

        Returns:
            Timeline HTML string
        """
        if not result.step_results:
            return ""

        # Calculate timeline data
        timeline_data = []
        for idx, step in enumerate(result.step_results):
            if step.start_time and step.end_time:
                start_offset = (step.start_time - result.start_time).total_seconds()
                duration = (step.end_time - step.start_time).total_seconds()
                timeline_data.append({
                    'index': idx + 1,
                    'name': step.name,
                    'start': start_offset,
                    'duration': duration,
                    'status': step.status
                })

        if not timeline_data:
            return ""

        # Calculate max time for scaling
        max_time = max(d['start'] + d['duration'] for d in timeline_data)

        return f"""
        <div class="section">
            <h2>⏱️ {self.texts['execution_timeline']}</h2>
            <p class="section-description">{self.texts['timeline_description']}</p>
            <div class="timeline-container">
                <div class="timeline-chart">
                    {"".join(self._build_timeline_bar(d, max_time) for d in timeline_data)}
                </div>
                <div class="timeline-legend">
                    <div class="legend-item">
                        <span class="legend-color" style="background-color: {self.colors['success']}"></span>
                        <span>{self.texts['passed'].split()[0]}</span>
                    </div>
                    <div class="legend-item">
                        <span class="legend-color" style="background-color: {self.colors['failure']}"></span>
                        <span>{self.texts['failed'].split()[0]}</span>
                    </div>
                    <div class="legend-item">
                        <span class="legend-color" style="background-color: {self.colors['skipped']}"></span>
                        <span>{self.texts['skipped'].split()[0]}</span>
                    </div>
                </div>
            </div>
        </div>
        """

    def _build_timeline_bar(self, data: Dict, max_time: float) -> str:
        """Build a single timeline bar.

        Args:
            data: Timeline data dict
            max_time: Maximum time for scaling

        Returns:
            Timeline bar HTML string
        """
        # Handle edge case where max_time is 0 or very small
        if max_time <= 0:
            left_percent = 0
            width_percent = 100
        else:
            left_percent = (data['start'] / max_time) * 100
            width_percent = max((data['duration'] / max_time) * 100, 1)  # At least 1% width

        status_color = {
            'success': self.colors['success'],
            'passed': self.colors['success'],
            'failure': self.colors['failure'],
            'failed': self.colors['failure'],
            'skipped': self.colors['skipped'],
            'error': self.colors['failure'],
        }.get(data['status'], self.colors['text_secondary'])

        return f"""
                <div class="timeline-bar-wrapper">
                    <div class="timeline-bar-label">{data['index']}. {self._escape_html(data['name'][:30])}</div>
                    <div class="timeline-bar">
                        <div class="timeline-bar-fill" style="
                            margin-left: {left_percent}%;
                            width: {width_percent}%;
                            background-color: {status_color};
                        " title="{self._escape_html(data['name'])}: {data['duration']:.3f}s"></div>
                    </div>
                    <div class="timeline-bar-time">{data['duration']:.3f}{self.texts['timeline_seconds']}</div>
                </div>
        """

    def _build_variable_flow_section(self, result: TestCaseResult) -> str:
        """Build variable flow diagram section.

        Args:
            result: Test case result

        Returns:
            Variable flow HTML string
        """
        # Collect variable flow data
        variable_events = []

        for idx, step in enumerate(result.step_results):
            # Check for created variables (extracted_vars)
            if step.extracted_vars:
                for var_name in step.extracted_vars.keys():
                    variable_events.append({
                        'step_index': idx + 1,
                        'step_name': step.name,
                        'var_name': var_name,
                        'action': 'created',
                        'value': str(step.extracted_vars[var_name])[:100]
                    })

            # Check for modified variables (variables_delta)
            if step.variables_delta:
                for var_name, change in step.variables_delta.items():
                    if isinstance(change, dict) and 'new_value' in change:
                        variable_events.append({
                            'step_index': idx + 1,
                            'step_name': step.name,
                            'var_name': var_name,
                            'action': 'modified',
                            'value': str(change['new_value'])[:100]
                        })

        if not variable_events:
            return ""

        return f"""
        <div class="section">
            <h2>🔄 {self.texts['variable_flow']}</h2>
            <p class="section-description">{self.texts['variable_flow_description']}</p>
            <div class="variable-flow-container">
                {"".join(self._build_variable_flow_event(event) for event in variable_events[:20])}
                {f"<div class='variable-flow-more'>... and {len(variable_events) - 20} more changes</div>" if len(variable_events) > 20 else ""}
            </div>
        </div>
        """

    def _build_variable_flow_event(self, event: Dict) -> str:
        """Build a single variable flow event.

        Args:
            event: Variable event dict

        Returns:
            Variable flow event HTML string
        """
        action_text = {
            'created': self.texts['variable_created'],
            'modified': self.texts['variable_modified'],
        }.get(event['action'], event['action'])

        action_color = {
            'created': self.colors['success'],
            'modified': self.colors['accent'],
        }.get(event['action'], self.colors['text_secondary'])

        return f"""
                <div class="variable-flow-event">
                    <div class="variable-flow-step">Step {event['step_index']}</div>
                    <div class="variable-flow-action" style="color: {action_color}">{action_text}</div>
                    <div class="variable-flow-name">{self._escape_html(event['var_name'])}</div>
                    <div class="variable-flow-value">= {self._escape_html(event['value'])}</div>
                </div>
        """

    def _build_dependency_section(self, result: TestCaseResult) -> str:
        """Build dependency relationship graph section.

        Args:
            result: Test case result

        Returns:
            Dependency graph HTML string
        """
        # Build dependency map based on variable usage
        dependencies = []
        var_to_step = {}  # Map variable name to step index that created it

        # First pass: find which steps create which variables
        for idx, step in enumerate(result.step_results):
            if step.extracted_vars:
                for var_name in step.extracted_vars.keys():
                    var_to_step[var_name] = idx

        # Second pass: find dependencies based on variable usage
        for idx, step in enumerate(result.step_results):
            deps = []
            # Check variables_snapshot to see which variables this step uses
            if step.variables_snapshot:
                for var_name in step.variables_snapshot.keys():
                    if var_name in var_to_step and var_to_step[var_name] < idx:
                        deps.append({
                            'var_name': var_name,
                            'step_index': var_to_step[var_name] + 1
                        })

            if deps:
                dependencies.append({
                    'step_index': idx + 1,
                    'step_name': step.name,
                    'depends_on': deps
                })

        if not dependencies:
            return f"""
        <div class="section">
            <h2>🔗 {self.texts['dependency_graph']}</h2>
            <p class="section-description">{self.texts['dependency_description']}</p>
            <p class="no-data">{self.texts['no_dependencies']}</p>
        </div>
        """

        return f"""
        <div class="section">
            <h2>🔗 {self.texts['dependency_graph']}</h2>
            <p class="section-description">{self.texts['dependency_description']}</p>
            <div class="dependency-container">
                {"".join(self._build_dependency_item(dep) for dep in dependencies)}
            </div>
        </div>
        """

    def _build_dependency_item(self, dep: Dict) -> str:
        """Build a single dependency item.

        Args:
            dep: Dependency dict

        Returns:
            Dependency item HTML string
        """
        deps_html = ", ".join(
            f"Step {d['step_index']} ({self._escape_html(d['var_name'])})"
            for d in dep['depends_on']
        )

        return f"""
                <div class="dependency-item">
                    <div class="dependency-step">
                        <strong>Step {dep['step_index']}:</strong>
                        {self._escape_html(dep['step_name'])}
                    </div>
                    <div class="dependency-deps">
                        {self.texts['step_depends_on']}: {deps_html}
                    </div>
                </div>
        """

    def _build_performance_analysis_section(self, result: TestCaseResult) -> str:
        """Build performance analysis section.

        Args:
            result: Test case result

        Returns:
            Performance analysis HTML string
        """
        # Collect performance data
        perf_data = []
        for step in result.step_results:
            if step.performance and step.performance.total_time > 0:
                perf_data.append({
                    'name': step.name,
                    'total_time': step.performance.total_time,
                    'dns_time': step.performance.dns_time,
                    'tcp_time': step.performance.tcp_time,
                    'tls_time': step.performance.tls_time,
                    'server_time': step.performance.server_time,
                    'download_time': step.performance.download_time,
                })

        if not perf_data:
            return ""

        # Calculate statistics
        total_time = sum(p['total_time'] for p in perf_data)
        avg_time = total_time / len(perf_data) if perf_data else 0

        # Sort by response time
        sorted_by_time = sorted(perf_data, key=lambda x: x['total_time'], reverse=True)
        slowest = sorted_by_time[:3]
        fastest = sorted_by_time[-3:] if len(sorted_by_time) >= 3 else sorted_by_time

        return f"""
        <div class="section">
            <h2>📊 {self.texts['performance_analysis']}</h2>
            <p class="section-description">{self.texts['performance_description']}</p>

            <div class="performance-overview">
                <div class="perf-stat-card">
                    <div class="perf-stat-value">{total_time:.2f}ms</div>
                    <div class="perf-stat-label">{self.texts['total_execution_time']}</div>
                </div>
                <div class="perf-stat-card">
                    <div class="perf-stat-value">{avg_time:.2f}ms</div>
                    <div class="perf-stat-label">{self.texts['average_response_time']}</div>
                </div>
            </div>

            <h4 class="subsection-title">🐌 {self.texts['slowest_steps']}</h4>
            <div class="performance-list">
                {"".join(f"<div class='perf-list-item'><strong>{self._escape_html(p['name'])}</strong>: {p['total_time']:.2f}ms</div>" for p in slowest)}
            </div>

            <h4 class="subsection-title">⚡ {self.texts['fastest_steps']}</h4>
            <div class="performance-list">
                {"".join(f"<div class='perf-list-item'><strong>{self._escape_html(p['name'])}</strong>: {p['total_time']:.2f}ms</div>" for p in reversed(fastest))}
            </div>

            <h4 class="subsection-title">📈 {self.texts['response_time_distribution']}</h4>
            <div class="response-time-chart">
                {"".join(self._build_perf_bar(p, max(p['total_time'] for p in perf_data)) for p in perf_data)}
            </div>
        </div>
        """

    def _build_perf_bar(self, perf: Dict, max_time: float) -> str:
        """Build a performance bar chart.

        Args:
            perf: Performance data dict
            max_time: Maximum time for scaling

        Returns:
            Performance bar HTML string
        """
        width_percent = (perf['total_time'] / max_time) * 100

        return f"""
                <div class="perf-bar-item">
                    <div class="perf-bar-label">{self._escape_html(perf['name'][:40])}</div>
                    <div class="perf-bar-track">
                        <div class="perf-bar-fill" style="width: {width_percent}%"></div>
                    </div>
                    <div class="perf-bar-value">{perf['total_time']:.2f}ms</div>
                </div>
        """

    def _build_footer(self) -> str:
        """Build footer section.

        Returns:
            Footer HTML string
        """
        return f"""
        <div class="footer">
            <p>{self.texts['generated_by']} <strong>Sisyphus API Engine</strong></p>
            <p>© {datetime.now().year} - {self.texts['footer_text']}</p>
        </div>
        </div>"""

    def _build_javascript(self) -> str:
        """Build JavaScript for interactivity.

        Returns:
            JavaScript string
        """
        return """
        <script>
        function toggleStep(header) {
            const details = header.nextElementSibling;
            details.classList.toggle('active');
        }

        function expandAll() {
            document.querySelectorAll('.step-details').forEach(el => {
                el.classList.add('active');
            });
        }

        function collapseAll() {
            document.querySelectorAll('.step-details').forEach(el => {
                el.classList.remove('active');
            });
        }

        // Auto-expand failed steps
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.step-item').forEach(item => {
                const status = item.querySelector('.step-meta div:first-child').textContent;
                if (status.includes('FAILED') || status.includes('ERROR')) {
                    item.querySelector('.step-details').classList.add('active');
                }
            });
        });
        </script>"""

    def _escape_html(self, text: str) -> str:
        """Escape HTML special characters.

        Args:
            text: Text to escape

        Returns:
            Escaped text
        """
        if not isinstance(text, str):
            text = str(text)
        return (
            text.replace("&", "&amp;")
            .replace("<", "&lt;")
            .replace(">", "&gt;")
            .replace('"', "&quot;")
            .replace("'", "&#x27;")
        )

    def save_html(self, result: TestCaseResult, output_path: str) -> None:
        """Save result as HTML file.

        Args:
            result: Test case result
            output_path: Output file path
        """
        html_content = self.to_html(result)

        with open(output_path, "w", encoding="utf-8") as f:
            f.write(html_content)
