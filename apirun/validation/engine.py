...
from apirun.validation.comparators import ComparatorError, get_comparator
...
class ValidationEngine:
        ...
        try:
            ...
            if not passed:
                ...
                error = (
                    error_message
                    if error_message
                    else self._generate_error_message(val_type, path, actual, expect)
                )
    ...
    def _validate_logical(
        ...
        if not sub_validations:
            return ValidationResult(
                ...
                error=error_message
                if error_message
                else f"Logical operator '{val_type}' requires sub_validations",
            )
        ...
        if val_type == 'and':
            ...
            if not passed:
                error = (
                    error_message
                    if error_message
                    else f'OR validation failed: All {len(sub_results)} sub-validations failed'
                )
            else:
                error = ''
            ...
            if passed_results:
                error = (
                    error_message
                    if error_message
                    else f'NOT validation failed: {len(passed_results)} sub-validations passed when all should fail'
                )
            else:
                error = ''
            ...
            return ValidationResult(
                ...
                error=error_message
                if error_message
                else f'Unknown logical operator: {val_type}',
            )
    ...
    def _generate_error_message(
        ...
        if val_type == 'eq':
            ...
            return 'Expected value to exist but got None or empty'
        elif val_type == 'between':
            try:
                return (
                    f'Expected {actual} to be between {expected[0]} and {expected[1]}'
                )
            except (TypeError, IndexError):
                return 'Expected value to be between min and max but validation failed'
        else:
            return (
                f'Validation failed: path={path}, expected={expected}, actual={actual}'
            )
