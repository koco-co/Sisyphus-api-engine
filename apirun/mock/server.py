...
class MockServer:
    ...
    def _setup_routes(self) -> None:
        ...
        def list_rules():
            ...
            return Response(
                json.dumps(rules_data, indent=2), mimetype='application/json'
            )
        ...
        def delete_rule(rule_name: str):
            ...
            if self.config.remove_rule(rule_name):
                return Response(
                    json.dumps({'status': 'deleted'}), mimetype='application/json'
                )
            return Response(
                json.dumps({'error': 'Rule not found'}, indent=2),
                status=404,
        ...
        def add_rule():
            ...
            try:
                ...
                return Response(
                    json.dumps({'status': 'added'}), mimetype='application/json'
                )
        ...
        def health_check():
            ...
            return Response(
                json.dumps({'status': 'ok', 'running': self._running}),
                mimetype='application/json',
            )
    ...
    def _handle_request(self, path: str) -> Response:
        ...
        rule = self.config.find_matching_rule(
            method, full_path, query_params, headers, body
        )
        ...
        if rule:
            ...
            mock_response = rule.get_response(
                method, full_path, query_params, headers, body
            )

            # Log condition evaluation if applicable
            if rule.condition:
                condition_met = rule.evaluate_condition(
                    method, full_path, query_params, headers, body
                )
    ...
    def _create_response(self, mock_response: MockResponse) -> Response:
        ...
        if mock_response.failure:
            if mock_response.failure.should_fail():
                self.logger.debug(
                    f'Simulating failure: {mock_response.failure.failure_type.value}'
                )
        ...
        response = Response(
            ...
            mimetype='application/json'
            if body and self._is_json(body)
            else 'text/plain',
        )
    ...
    def _create_failure_response(self, failure: FailureConfig) -> Response:
        ...
        if failure.failure_type == FailureType.TIMEOUT:
            ...
            return Response(
                response='{invalid json', status=200, mimetype='application/json'
            )
            ...
            return Response(
                json.dumps({'error': 'Unknown failure type'}),
                status=500,
                mimetype='application/json',
            )
    ...
    def start(self, blocking: bool = False) -> None:
        ...
        if blocking:
            ...
            self.logger.info(
                f'Mock server started on http://{self.config.host}:{self.config.port}'
            )
...
def create_regex_rule(
    ...
    """
    matcher = RequestMatcher(
        method=method, path=path_pattern, match_type=MatchType.REGEX
    )
