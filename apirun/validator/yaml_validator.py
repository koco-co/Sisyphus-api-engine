...
class ValidationResult:
    ...
    unknown_keywords: List[Tuple[str, str]] = field(default_factory=list)  # (keyword, location)
    ...
    def has_errors(self) -> bool:
        ...
        return bool(
            self.syntax_errors or self.missing_fields or self.unknown_keywords
        )
                ...
                if isinstance(poll_config, dict):
                    for key in poll_config.keys():
                        if key not in self.VALID_POLL_CONFIG_KEYWORDS:
                            result.unknown_keywords.append(
                                (key, f"{step_location}.poll_config")
                            )
...
class TerminalFormatter:
    ...
    def print_summary(results: List[ValidationResult]) -> None:
        ...
        if invalid == 0:
            print(
                f"  {Colors.GREEN}{Colors.BOLD}所有文件验证通过！{Colors.RESET}"
            )
        else:
            print(
                f"  {Colors.RED}{Colors.BOLD}部分文件验证失败{Colors.RESET}"
            )
            ...
            if path.suffix in (".yaml", ".yml"):
                ...
                print(
                    TerminalFormatter.warning(
                        f"跳过非 YAML 文件: {path_str}"
                    )
                )
        elif path.is_dir():
            found_files = list(path.glob("**/*.yaml")) + list(
                path.glob("**/*.yml")
            )
            if found_files:
