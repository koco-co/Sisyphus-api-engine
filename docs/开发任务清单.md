...
| MDL-015 | RequestStepParams 互斥校验 | `[x]` | json/data/files 三者互斥的 model_validator |
| MDL-016 | Config csv_datasource 与 ddts 互斥校验 | `[x]` | CaseModel 级别的 model_validator |
...
| REQ-018 | files 参数 MinIO 路径下载 | `[x]` | Value 为 MinIO 路径时自动下载为临时文件再上传 |
...
| RUN-012 | custom 步骤: 调用自定义关键字执行器 | `[x]` | CST-001～007 已接入，支持 extract |
...
| 编号 | 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|------|
| RPT-008 | 生成 Allure JSON 结果文件 | `allure_reporter.py` | `[x]` | 写入 --allure-dir 目录 |
| RPT-009 | 步骤映射为 Allure Step | `allure_reporter.py` | `[x]` | |
| RPT-010 | 断言结果映射为 Attachment | `allure_reporter.py` | `[x]` | |

### 13.4 HTML 报告 (`-O html`)

| 编号 | 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|------|
| RPT-011 | 生成自包含 HTML 报告文件 | `html_reporter.py` | `[x]` | 写入 --html-dir 目录 |
| RPT-012 | 包含摘要/步骤详情/断言/响应数据 | `html_reporter.py` | `[x]` | |
...
| CLI-013 | 集成 allure_reporter 模块 | `[x]` | -O allure 时写入 --allure-dir |
| CLI-014 | 集成 html_reporter 模块 | `[x]` | -O html 时写入 --html-dir |
...
| 编号 | 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|------|
| WS-001 | EventPublisher 发布器 | `publisher.py` | `[x]` | 场景开始/步骤开始/步骤完成/场景完成 |
| WS-002 | 推送消息格式 | `publisher.py` | `[x]` | event_type/step_index/status/timestamp/data |
| WS-003 | WebSocket 连接失败降级 | `publisher.py` | `[x]` | 不影响用例执行 |
| WS-004 | runner 集成 publisher | `runner.py` | `[x]` | 可选注入 publisher 实例 |
...
| ERR-004 | 引擎级错误: 立即终止, status=error | `runner.py` | `[x]` | load_case 使用 EngineError，CLI 在 JSON/Text 模式下输出结构化错误并 exit 1 |
| ERR-005 | 步骤级错误: 记录并继续, status=error | `runner.py` | `[x]` | RUN-015 步骤 try/except，记录 error 并继续 |
| ERR-006 | 断言失败: 记录并继续, status=failed | `runner.py` | `[x]` | step_status=failed，不中断后续步骤 |
| ERR-007 | 提取失败: 使用 default 或记录失败 | `extractor.py` | `[x]` | default 写入变量；无 default 时 status=failed |
...
| TST-029 | test_custom_executor: 自定义关键字 | `test_executor_custom.py` | `[x]` | 已覆盖注册/执行/KEYWORD_NOT_FOUND/执行异常/变量渲染 |
...
| TST-034 | test_text_reporter: rich 输出 | `test_text_reporter.py` | `[x]` | 最小结果与 verbose 渲染 |
| TST-035 | test_runner_integration: 完整场景端到端 | `test_runner_integration.py` | `[x]` | load_case + run_case full_step_types |
...
| YML-002 | simple_get.yaml | `simple_get.yaml` | `[x]` | 最简 GET 请求 + status_code 断言 |
| YML-003 | multi_step_e2e.yaml | `multi_step_e2e.yaml` | `[x]` | 注册→登录→查看信息 多步骤 |
| YML-004 | data_driven_inline.yaml | `data_driven_inline.yaml` | `[x]` | ddts 内联数据驱动 |
| YML-005 | data_driven_csv.yaml + data.csv | `data_driven_csv.yaml` | `[x]` | CSV 文件数据驱动 |
| YML-006 | variable_extraction.yaml | `variable_extraction.yaml` | `[x]` | 变量提取 + 跨步骤引用 |
| YML-007 | all_comparators.yaml | `all_comparators.yaml` | `[x]` | 多种比较器断言示例 |
| YML-008 | error_cases.yaml | `error_cases.yaml` | `[x]` | KEYWORD_NOT_FOUND 等错误场景 |
| YML-009 | pre_post_sql.yaml | `pre_post_sql.yaml` | `[x]` | 前置/后置 SQL |
...
| 编号 | 任务 | 状态 | 说明 |
|------|------|------|------|
| PUB-001 | CHANGELOG.md 维护 | `[x]` | 记录版本变更 |
| PUB-002 | pypi_publish.sh 发布脚本 | `[x]` | 脚本已存在，已补充 TestPyPI/dry-run 支持并在 README 文档化 |
| PUB-003 | version 管理 (0.1.0 → 1.0.0) | `[x]` | pyproject.toml 2.2.0 + __init__.py 一致 |
| PUB-004 | LICENSE 文件 | `[x]` | MIT 已存在 |
| PUB-005 | examples/ 示例用例 | `[x]` | 3 个 YAML + README，基于 httpbin |
...
|------|--------|--------|--------|--------|--------|
| 1. 基础设施 | 7 | 7 | 0 | 0 | 100% |
| 2. YAML 输入模型 | 16 | 16 | 0 | 0 | 100% |
| 3. JSON 输出模型 | 14 | 14 | 0 | 0 | 100% |
| 4. YAML 解析器 | 9 | 9 | 0 | 0 | 100% |
| 5. 变量系统 | 17 | 17 | 0 | 0 | 100% |
| 6. HTTP 请求执行器 | 18 | 18 | 0 | 0 | 100% |
| 7. 断言验证引擎 | 27 | 27 | 0 | 0 | 100% |
| 8. 变量提取器 | 10 | 10 | 0 | 0 | 100% |
| 9. 数据库执行器 | 11 | 11 | 0 | 0 | 100% |
| 10. 自定义关键字执行器 | 7 | 7 | 0 | 0 | 100% |
| 11. 场景运行器 | 21 | 21 | 0 | 0 | 100% |
| 12. 数据驱动 | 8 | 8 | 0 | 0 | 100% |
| 13. 结果收集器 | 12 | 12 | 0 | 0 | 100% |
| 14. CLI | 15 | 15 | 0 | 0 | 100% |
| 15. WebSocket | 4 | 4 | 0 | 0 | 100% |
| 16. 错误处理 | 7 | 7 | 0 | 0 | 100% |
| 17. 单元测试 | 35 | 35 | 0 | 0 | 100% |
| 18. YAML 测试用例 | 9 | 9 | 0 | 0 | 100% |
| 19. 发布准备 | 5 | 5 | 0 | 0 | 100% |
| **合计** | **252** | **252** | **0** | **0** | **100%** |
