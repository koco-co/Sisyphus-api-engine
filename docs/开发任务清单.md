...
| 编号 | 任务 | 状态 | 说明 |
|------|------|------|------|
| OUT-001 | ErrorInfo 错误信息模型 | `[ ]` | code/message/detail |
| OUT-002 | ExecutionSummary 执行摘要模型 | `[ ]` | 16 个统计字段：total_steps/passed_steps/.../total_data_driven_runs |
| OUT-003 | EnvironmentInfo 环境快照模型 | `[ ]` | name/base_url/variables |
| OUT-004 | RequestDetail 请求详情模型 | `[ ]` | method/url/headers/params/body/body_type/timeout/allow_redirects/verify_ssl |
| OUT-005 | ResponseDetail 响应详情模型 | `[ ]` | status_code/headers/body/body_size/response_time/cookies |
| OUT-006 | AssertionResult 断言结果模型 | `[ ]` | target/expression/comparator/expected/actual/status/message |
| OUT-007 | ExtractResult 提取结果模型 | `[ ]` | name/type/expression/scope/value/status |
| OUT-008 | DbDetail 数据库详情模型 | `[ ]` | datasource/sql/sql_rendered/row_count/columns/rows/execution_time |
| OUT-009 | CustomDetail 自定义详情模型 | `[ ]` | keyword_name/parameters_input/return_value/execution_time |
| OUT-010 | StepResult 步骤结果模型 | `[ ]` | 12 个通用字段 + 条件详情字段 |
| OUT-011 | LogEntry 日志条目模型 | `[ ]` | timestamp/level/message/step_index |
| OUT-012 | DataDrivenRun 单轮结果模型 | `[ ]` | run_index/parameters/status/duration/summary/steps |
| OUT-013 | DataDrivenResult 数据驱动结果模型 | `[ ]` | enabled/source/dataset_name/total_runs/passed_runs/failed_runs/pass_rate/runs |
| OUT-014 | ExecutionResult 顶层结果模型 | `[ ]` | 14 个顶层字段 + model_dump() 可序列化 |
...
| 编号 | 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|------|
| PAR-001 | yaml_parser: 读取 YAML 并解析为 dict | `yaml_parser.py` | `[ ]` | 封装 yaml.safe_load，统一异常处理 |
| PAR-002 | yaml_parser: 文件不存在时错误码 FILE_NOT_FOUND | `yaml_parser.py` | `[ ]` | 抛出带 error code 的 EngineError |
| PAR-003 | yaml_parser: YAML 语法错误时错误码 YAML_PARSE_ERROR | `yaml_parser.py` | `[ ]` | 捕获 yaml.YAMLError |
| PAR-004 | yaml_parser: Pydantic 校验失败时错误码 YAML_VALIDATION_ERROR | `yaml_parser.py` | `[ ]` | 捕获 ValidationError |
| PAR-005 | yaml_parser: 返回 CaseModel 实例 | `yaml_parser.py` | `[ ]` | `parse_yaml(path) -> CaseModel` |
| PAR-006 | csv_parser: 读取 CSV 并解析为 list[dict] | `csv_parser.py` | `[ ]` | 第一行为表头，后续行为数据 |
| PAR-007 | csv_parser: 文件不存在时错误码 CSV_FILE_NOT_FOUND | `csv_parser.py` | `[ ]` | |
| PAR-008 | csv_parser: 格式错误时错误码 CSV_PARSE_ERROR | `csv_parser.py` | `[ ]` | |
| PAR-009 | EngineError 统一异常类 | `apirun/errors.py` | `[ ]` | code/message/detail 属性，所有模块复用 |
...
| VAR-009 | 全局参数函数调用支持 | `[ ]` | `{{方法名}}` / `{{方法名(参数)}}` 查询平台注册的全局参数函数 |
| VAR-010 | 变量池管理器 (VariablePool) | `[ ]` | 分层变量池：data_driven > extracted > scenario > environment > global_params |
| VAR-011 | 变量作用域 scope 支持 | `[ ]` | global 写入全局池, environment 写入环境池 |
...
| 编号 | 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|------|
| VLD-001 | comparators: `eq` (等于) | `comparators.py` | `[ ]` | |
| VLD-002 | comparators: `neq` (不等于) | `comparators.py` | `[ ]` | |
| VLD-003 | comparators: `gt` (大于) | `comparators.py` | `[ ]` | |
| VLD-004 | comparators: `gte` (大于等于) | `comparators.py` | `[ ]` | |
| VLD-005 | comparators: `lt` (小于) | `comparators.py` | `[ ]` | |
| VLD-006 | comparators: `lte` (小于等于) | `comparators.py` | `[ ]` | |
| VLD-007 | comparators: `contains` (包含) | `comparators.py` | `[ ]` | 字符串/列表 |
| VLD-008 | comparators: `not_contains` (不包含) | `comparators.py` | `[ ]` | |
| VLD-009 | comparators: `startswith` (前缀) | `comparators.py` | `[ ]` | |
| VLD-010 | comparators: `endswith` (后缀) | `comparators.py` | `[ ]` | |
| VLD-011 | comparators: `matches` (正则) | `comparators.py` | `[ ]` | re.search |
| VLD-012 | comparators: `type_match` (类型匹配) | `comparators.py` | `[ ]` | int/str/list/dict/bool/null |
| VLD-013 | comparators: `length_eq` (长度等于) | `comparators.py` | `[ ]` | |
| VLD-014 | comparators: `length_gt` (长度大于) | `comparators.py` | `[ ]` | |
| VLD-015 | comparators: `length_lt` (长度小于) | `comparators.py` | `[ ]` | |
| VLD-016 | comparators: `is_null` (为空) | `comparators.py` | `[ ]` | |
| VLD-017 | comparators: `is_not_null` (不为空) | `comparators.py` | `[ ]` | |
| VLD-018 | validator: target=json (JSONPath 提取) | `validator.py` | `[ ]` | 使用 jsonpath-ng |
| VLD-019 | validator: target=header (响应头提取) | `validator.py` | `[ ]` | 按名称取值 |
| VLD-020 | validator: target=cookie (Cookie 提取) | `validator.py` | `[ ]` | 按名称取值 |
| VLD-021 | validator: target=status_code | `validator.py` | `[ ]` | 直接取状态码 |
| VLD-022 | validator: target=response_time | `validator.py` | `[ ]` | 直接取响应耗时 ms |
| VLD-023 | validator: target=env_variable | `validator.py` | `[ ]` | 从变量池取值 |
| VLD-024 | validator: expected 中 `{{变量}}` 替换 | `validator.py` | `[ ]` | 渲染后再比较 |
| VLD-025 | validator: 返回 AssertionResult 结构 | `validator.py` | `[ ]` | target/expression/comparator/expected/actual/status/message |
| VLD-026 | validator: 失败时返回自定义 message 或默认消息 | `validator.py` | `[ ]` | |
| VLD-027 | validator: 数据库结果断言 (`$.length` 内置) | `validator.py` | `[ ]` | db_result target |
...
| 编号 | 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|------|
| EXT-001 | extractor: type=json (JSONPath 提取) | `extractor.py` | `[ ]` | jsonpath-ng 求值 |
| EXT-002 | extractor: type=header (响应头提取) | `extractor.py` | `[ ]` | 按名称取值 |
| EXT-003 | extractor: type=cookie (Cookie 提取) | `extractor.py` | `[ ]` | 按名称取值 |
| EXT-004 | extractor: scope=global 写入全局池 | `extractor.py` | `[ ]` | |
| EXT-005 | extractor: scope=environment 写入环境池 | `extractor.py` | `[ ]` | |
| EXT-006 | extractor: 提取失败使用 default 默认值 | `extractor.py` | `[ ]` | |
| EXT-007 | extractor: 无 default 时错误码 EXTRACT_FAILED | `extractor.py` | `[ ]` | |
| EXT-008 | extractor: 返回 ExtractResult 结构 | `extractor.py` | `[ ]` | name/type/expression/scope/value/status |
| EXT-009 | extractor: source_variable 指定数据源 | `extractor.py` | `[ ]` | 默认上一个 request 响应 |
| EXT-010 | extractor: db_result 类型 (数据库结果提取) | `extractor.py` | `[ ]` | JSONPath 从 rows JSON 数组提取 |
...
| 编号 | 任务 | 状态 | 说明 |
|------|------|------|------|
| DB-001 | datasource 引用变量名解析 | `[ ]` | 从 .sisyphus/config.yaml 或变量池查找连接配置 |
| DB-002 | MySQL 连接 (pymysql) | `[ ]` | host/port/user/password/database |
| DB-003 | PostgreSQL 连接 (psycopg2) | `[ ]` | host/port/user/password/database |
| DB-004 | SQL 语句 `{{变量}}` 替换 | `[ ]` | 使用 render_template |
| DB-005 | 执行 SQL 查询并返回 JSON 数组 | `[ ]` | 列名 → dict key |
| DB-006 | 返回 db_detail 结构 | `[ ]` | datasource/sql/sql_rendered/row_count/columns/rows/execution_time |
| DB-007 | DB_CONNECTION_ERROR 错误码 | `[ ]` | 连接失败 |
| DB-008 | DB_QUERY_ERROR 错误码 | `[ ]` | SQL 执行失败 |
| DB-009 | DB_DATASOURCE_NOT_FOUND 错误码 | `[ ]` | 数据源未找到 |
| DB-010 | db.extract 结果提取 | `[ ]` | JSONPath 从查询结果中提取变量 |
| DB-011 | db.validate 结果断言 | `[ ]` | JSONPath + `$.length` 内置表达式 |
...
| 编号 | 任务 | 状态 | 说明 |
|------|------|------|------|
| CST-001 | 根据 keyword_name 查找自定义关键字类 | `[ ]` | 动态加载 Keyword 子类 |
| CST-002 | 将 parameters 传入 execute() | `[ ]` | **kwargs 展开 |
| CST-003 | 捕获 return_value | `[ ]` | 供后续 extract 使用 |
| CST-004 | 返回 custom_detail 结构 | `[ ]` | keyword_name/parameters_input/return_value/execution_time |
| CST-005 | KEYWORD_NOT_FOUND 错误码 | `[ ]` | 关键字未注册 |
| CST-006 | KEYWORD_EXECUTION_ERROR 错误码 | `[ ]` | 执行异常 |
| CST-007 | 从 extract 字段提取变量 | `[ ]` | custom 步骤可内嵌 extract |
...
| 编号 | 任务 | 状态 | 说明 |
|------|------|------|------|
| RUN-008 | enabled=false 时 status=skipped | `[~]` | 当前跳过但未生成 skipped StepResult |
| RUN-009 | assertion 步骤: 调用断言验证引擎 | `[ ]` | 独立断言步骤执行 |
| RUN-010 | extract 步骤: 调用变量提取器 | `[ ]` | 独立提取步骤执行 |
| RUN-011 | db 步骤: 调用数据库执行器 | `[ ]` | 数据库步骤执行 |
| RUN-012 | custom 步骤: 调用自定义关键字执行器 | `[ ]` | 自定义关键字执行 |
| RUN-013 | request 步骤内联 extract 执行 | `[ ]` | 请求后立即提取变量 |
| RUN-014 | request 步骤内联 validate 执行 | `[ ]` | 请求后立即断言 |
| RUN-015 | 步骤异常捕获: status=error, 不中断后续 | `[ ]` | try/except 包装每步 |
| RUN-016 | 场景整体 status 计算 | `[ ]` | 基于所有步骤+断言结果推导 passed/failed/error |
| RUN-017 | assertion_results 统计到 summary | `[ ]` | total_assertions/passed_assertions/failed_assertions/pass_rate |

### 11.3 前置/后置处理

| 编号 | 任务 | 状态 | 说明 |
|------|------|------|------|
| RUN-018 | pre_sql 前置 SQL 执行 | `[ ]` | 调用数据库执行器, 在 teststeps 之前 |
| RUN-019 | post_sql 后置 SQL 执行 | `[ ]` | 在 teststeps 之后执行 (无论成功失败) |

### 11.4 数据驱动集成

| 编号 | 任务 | 状态 | 说明 |
|------|------|------|------|
| RUN-020 | 检测 ddts 字段启动数据驱动模式 | `[ ]` | ddts 存在时循环执行 |
| RUN-021 | 检测 csv_datasource 启动 CSV 数据驱动 | `[ ]` | 解析 CSV 后循环执行 |
| RUN-022 | 每轮注入数据驱动变量到变量池 | `[ ]` | 优先级最高 |
| RUN-023 | 合并 DataDrivenResult 到顶层 | `[ ]` | data_driven 字段 |

### 11.5 日志收集

| 编号 | 任务 | 状态 | 说明 |
|------|------|------|------|
| RUN-024 | LogCollector 日志收集器 | `[ ]` | 场景开始/步骤开始/步骤完成/场景完成 |
| RUN-025 | 生成 LogEntry (timestamp/level/message/step_index) | `[ ]` | 写入 logs 数组 |
| RUN-026 | DEBUG 级别: 请求 URL/方法详情 | `[ ]` | 仅 verbose 模式 |

### 11.6 重构: 使用输出数据模型

| 编号 | 任务 | 状态 | 说明 |
|------|------|------|------|
| RUN-027 | runner 返回 ExecutionResult 模型 (非裸 dict) | `[ ]` | 使用 OUT-014 定义的 Pydantic 模型 |
| RUN-028 | StepResult 模型化 | `[ ]` | 各步骤统一返回 StepResult |
...
| 编号 | 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|------|
| DDT-001 | YAML 内联数据驱动 (ddts 字段解析) | `driver.py` | `[ ]` | 遍历 ddts.parameters 数组 |
| DDT-002 | CSV 文件数据驱动 (csv_datasource) | `driver.py` | `[ ]` | 调用 csv_parser 解析 |
| DDT-003 | 每行数据 = 一次独立场景执行 | `driver.py` | `[ ]` | 克隆 teststeps + 注入变量 |
| DDT-004 | 数据集字段通过 `{{字段名}}` 注入 | `driver.py` | `[ ]` | 合并到变量池最高优先级 |
| DDT-005 | 返回 DataDrivenResult 结构 | `driver.py` | `[ ]` | enabled/source/dataset_name/total_runs/... |
| DDT-006 | 每轮 runs: run_index/parameters/status/duration/summary/steps | `driver.py` | `[ ]` | |
| DDT-007 | source 标识: yaml_inline / csv_file | `driver.py` | `[ ]` | |
| DDT-008 | 汇总所有轮次到顶层 summary 和 status | `driver.py` | `[ ]` | |
...
| 编号 | 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|------|
| RPT-001 | JSON 输出到 stdout | `json_reporter.py` | `[~]` | 当前在 cli.py 中直接 json.dumps，未独立为 Reporter |
| RPT-002 | ensure_ascii=False 支持中文 | `json_reporter.py` | `[x]` | 已实现 |
| RPT-003 | 时间字段 ISO 8601 含时区 | `json_reporter.py` | `[~]` | 当前用 UTC isoformat，缺时区偏移 |
| RPT-004 | 引擎级异常时输出 error 对象 | `json_reporter.py` | `[ ]` | status=error + error 字段 |

### 13.2 Text 报告 (`-O text`)

| 编号 | 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|------|
| RPT-005 | rich 库渲染终端美化输出 | `text_reporter.py` | `[ ]` | Table/Panel/Console |
| RPT-006 | 场景名称/状态/耗时/步骤概览 | `text_reporter.py` | `[ ]` | 摘要信息 |
| RPT-007 | -v/--verbose 控制详细程度 | `text_reporter.py` | `[ ]` | verbose=True 显示请求/响应详情 |
...
| CLI-011 | 集成 json_reporter 模块 | `[ ]` | 替换当前内联 json.dumps |
| CLI-012 | 集成 text_reporter 模块 | `[ ]` | 替换当前最简输出 |
| CLI-013 | 集成 allure_reporter 模块 | `[ ]` | 调用 allure_reporter.generate() |
| CLI-014 | 集成 html_reporter 模块 | `[ ]` | 调用 html_reporter.generate() |
| CLI-015 | 断言失败时 exit code 0, status=failed | `[ ]` | 区分执行失败和引擎异常 |
...
| 编号 | 任务 | 文件 | 状态 | 说明 |
|------|------|------|------|------|
| ERR-001 | EngineError 统一异常基类 | `apirun/errors.py` | `[ ]` | code + message + detail |
| ERR-002 | 引擎级错误码常量 | `apirun/errors.py` | `[ ]` | FILE_NOT_FOUND / YAML_PARSE_ERROR / YAML_VALIDATION_ERROR / CSV_* / ENGINE_INTERNAL_ERROR / TIMEOUT_ERROR |
| ERR-003 | 步骤级错误码常量 | `apirun/errors.py` | `[ ]` | REQUEST_* / ASSERTION_FAILED / EXTRACT_FAILED / DB_* / KEYWORD_* / VARIABLE_* |
...
| TST-024 | test_output_models: ExecutionResult 模型 | `test_output_models.py` | `[ ]` | |
| TST-025 | test_comparators: 17 种比较器 | `test_comparators.py` | `[ ]` | |
| TST-026 | test_validator: 6 种 target | `test_validator.py` | `[ ]` | |
| TST-027 | test_extractor: json/header/cookie 提取 | `test_extractor.py` | `[ ]` | |
| TST-028 | test_db_executor: MySQL/PostgreSQL 执行 | `test_db_executor.py` | `[ ]` | |
| TST-029 | test_custom_executor: 自定义关键字 | `test_custom_executor.py` | `[ ]` | |
| TST-030 | test_data_driven: YAML 内联驱动 | `test_data_driven.py` | `[ ]` | |
| TST-031 | test_data_driven: CSV 文件驱动 | `test_data_driven.py` | `[ ]` | |
| TST-032 | test_csv_parser: CSV 解析 | `test_csv_parser.py` | `[ ]` | |
| TST-033 | test_yaml_parser: 统一解析器 | `test_yaml_parser.py` | `[ ]` | |
